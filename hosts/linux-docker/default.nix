...
# Docker VM NixOS Configuration
#
# This configuration is for a Docker host running as an Incus VM.
# It includes automatic mounting of the NAS /mnt/app_config share for container volumes.
#
...
{
  imports = [
    # Include the default incus configuration for VMs running as Incus guests
  ...
  # Host identification
  networking.hostName = myvars.hosts.docker.hostname;
  ...
  # Network configuration for Incus VM
  networking = {
    dhcpcd.enable = false;
    useDHCP = false;
  ...
  # NAS share mounting configuration for Docker volumes
